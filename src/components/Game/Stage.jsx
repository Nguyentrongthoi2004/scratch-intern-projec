...
const Stage = ({ 
  ...
  waitTimer, friend, messageColor, tapEffect,
  ...
  // --- MESSAGE COLORS ---
  const getMessageColor = (colorName) => {
     switch(colorName?.toLowerCase()) {
         case 'red': return 'text-red-500 border-red-500 shadow-red-500/50';
         case 'blue': return 'text-blue-500 border-blue-500 shadow-blue-500/50';
         case 'green': return 'text-green-500 border-green-500 shadow-green-500/50';
         case 'yellow': return 'text-yellow-400 border-yellow-400 shadow-yellow-400/50';
         default: return 'text-orange-400 border-orange-400 shadow-orange-400/50';
     }
  };
  const msgClasses = getMessageColor(messageColor);
    ...
    `}>
      ...
      {friend && friend.visible && CHAR_CONFIG[friend.id] && (
        <div className="absolute z-10 w-32 h-32 will-change-transform" style={friendStyle}>
           <img 
             src={`assets/images/characters/${friend.id}/${friend.isAttacking ? CHAR_CONFIG[friend.id].say.fileName : CHAR_CONFIG[friend.id].idle.fileName}`}
             alt="Friend"
           ...
           <div className="absolute top-0 flex justify-center w-full -mt-8 animate-bounce">
              <span className="text-2xl filter drop-shadow-lg">{friend.isAttacking ? '⚔️' : '💖'}</span>
           </div>
        </div>
      )}

      {(status === 'hurt' || status === 'push') && !friend && (
      ...
      {/* TAP EFFECT */}
      {tapEffect && (
          <div className="absolute z-50 w-20 h-20 -translate-x-1/2 -translate-y-1/2 rounded-full border-4 border-white opacity-0 animate-ping top-1/2 left-1/2 pointer-events-none"></div>
      )}
      ...
      <div className="absolute z-20 w-32 h-32 will-change-transform" style={characterStyle}>
        ...
        {status === 'throw' && (
          <div className={`absolute flex items-center justify-center w-10 h-10 bg-slate-800 border-2 rounded-full -top-10 -right-8 animate-bounce z-20 ${msgClasses}`}>
            {IconMail ? <IconMail className={`w-5 h-5 ${msgClasses.split(' ')[0]}`} /> : <span>✉️</span>}
          </div>
        )}
